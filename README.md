...
- **Get missing localization languages**: Returns a list of languages that haven't been localized yet for the specified content. If you are using Strapi v4 specify the `Strapi version` input as `v4` to use the Strapi v4-compatible REST flow. The `Strapi version` input is optional and defaults to `v5`.
